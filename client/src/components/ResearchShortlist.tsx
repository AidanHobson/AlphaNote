...
export default function ResearchShortlist({ onResearch }: { onResearch: (symbol: string) => void }) {
  ...
  return (
    <Card
      ...
      sub="live candidates ranked by composite signal strength (attention · short-squeeze setup · momentum · insider buying) — what to research, not what to buy"
      style={{ marginTop: 16 }}
    >
      {!data ? <SkeletonLines lines={5} /> : (
        <table className="mtable">
          <thead><tr><th>#</th><th>Ticker</th><th className="num">Score</th><th>Why it ranks</th><th className="num">Price</th><th /></tr></thead>
          <tbody>

...
export default function ResearchShortlist({ onResearch }: { onResearch: (symbol: string) => void }) {
  ...
  return (
    <Card
      ...
      sub="candidates from Reddit-trending names, market-wide insider buying, the day's biggest movers, and US-listed Chinese tech and robotics/autonomy ADRs — ranked by composite signal strength — what to research, not what to buy"
      style={{ marginTop: 16 }}
    >
          ...
          <tbody>
            {(data.candidates || []).map((c, i) => (
              <tr key={c.symbol} role="button" tabIndex={0} title={`Research ${c.symbol}`}
                ...
                <td><span className={`badge ${c.source === 'insider buying' ? 'up' : 'flat'}`}>{c.source === 'insider buying' ? '★ insider' : c.source === 'big mover' ? '⇅ mover' : c.source === 'china robotics' ? '🤖 china robo' : c.source === 'china tech' ? '🇨🇳 china' : '🔥 trending'}</span></td>
